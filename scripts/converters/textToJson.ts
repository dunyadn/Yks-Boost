...
function parseTextFile(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (currentQuestion && currentQuestion.content !== undefined) {
      ...
      if (
        !line.match(/^[A-E][):]/) &&
        !line.match(/(?:Cevap|Doğru|Çözüm|Konu):/i)
      ) {
...
function parseArgs(): {
  ...
  if (args.length < 2) {
    console.error(
      "Kullanım: npx tsx scripts/converters/textToJson.ts <input-file> <output-file> [options]",
    );
    console.error("\nSeçenekler:");
    ...
    console.error(
      "  --category <category>    Kategori (Matematik, Fizik, Türkçe, vb.)",
    );
    console.error("  --description <desc>     Paket açıklaması");
    console.error("\nÖrnek:");
    console.error(
      '  npx tsx scripts/converters/textToJson.ts input.txt output.json --package-name "TYT 2026 Tarih" --exam-type TYT --year 2026 --category Tarih',
    );
    process.exit(1);
  }
...
function main() {
  try {
    ...
    console.log(
      `✅ ${packageData.questions.length} soru başarıyla parse edildi`,
    );

    // Validasyon
    const invalidQuestions = packageData.questions.filter(
      (q) =>
    ...
    if (invalidQuestions.length > 0) {
      console.warn(
        `⚠️  Uyarı: ${invalidQuestions.length} soru eksik bilgi içeriyor`,
      );
    }
