...
function parseFilenameMetadata(filename: string): {
  ...
  const examTypeMatch = nameWithoutExt.match(
    /(?:^|[_\-\s])(MSÜ|TYT|AYT|YKS|MEBI|Mebi)(?:[_\-\s]|$)/i,
  );
...
function parseQuestions(content: string): ParsedQuestion[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const multiOptionMatch = line.match(
      /A\)\s*([^B]+)\s*B\)\s*([^C]+)\s*C\)\s*([^D]+)\s*D\)\s*([^E]+)\s*E\)\s*(.+)/,
    );
    if (multiOptionMatch) {
      currentQuestion.options = [
    ...
    const answerMatch = line.match(
      /(?:Cevap|Doğru(?:\s+Cevap)?|CEVAP):\s*([A-E])/i,
    );
    if (answerMatch) {
    ...
    if (
      currentQuestion.content !== undefined &&
      !line.match(/^[A-E][):]/) &&
      !collectingSolution
    ) {
 ...
 * Validate package
 */
function validatePackage(pkg: QuestionPackage): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  if (pkg.totalQuestions !== pkg.questions.length) {
    errors.push(
      `Soru sayısı uyumsuz: totalQuestions=${pkg.totalQuestions}, questions.length=${pkg.questions.length}`,
    );
  }
  ...
  pkg.questions.forEach((q, index) => {
    ...
    if (!q.solution || q.solution.trim().length === 0) {
      ...
      errors.push(
        `Soru ${index + 1}: Çözüm çok kısa (en az 50 karakter olmalı)`,
      );
    }
...
async function processFile(
  ...
  try {
    ...
    console.log(
      `  📊 Metadata: ${metadata.examType} ${metadata.year} ${metadata.lesson}`,
    );

    // Convert to package format (with AI solutions)
    console.log("  🔄 JSON paketi oluşturuluyor...");
    const pkg = await convertToPackageFormat(
      parsedQuestions,
      metadata,
      filePath,
      apiKey,
...
async function main() {
  ...
  if (args.length < 2) {
    console.error(
      "Kullanım: npx tsx scripts/converters/comprehensiveConverter.ts <source-dir> <output-dir> [--api-key <key>]",
    );
    console.error("\nÖrnek:");
    console.error(
      "  npx tsx scripts/converters/comprehensiveConverter.ts ./data/text-files ./data/questions",
    );
    console.error(
      "  npx tsx scripts/converters/comprehensiveConverter.ts ./data/text-files ./data/questions --api-key YOUR_GEMINI_API_KEY",
    );
    process.exit(1);
  }
  ...
  if (apiKeyIndex !== -1 && args[apiKeyIndex + 1]) {
    ...
    console.log(
      "📝 Şablon tabanlı çözüm oluşturma aktif (daha detaylı çözümler için --api-key kullanın)",
    );
  }
